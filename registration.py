...
class Registration:
    ...
    def __init__(self, ray_type: TypeAlias, image_type: TypeAlias, volume, *, source_position: torch.Tensor):
    ...
    def set_image_from_random_drr(self, *, image_size: torch.Tensor, drr_alpha: float):
        ...
        self.set_image(self.image_type(self.generate_drr(true_theta, image_size=image_size, alpha=drr_alpha),
            generate_mip_levels=True))
        debug.toc()
        return true_theta

    def set_image_from_drr(self, theta, *, image_size: torch.Tensor, drr_alpha: float):
        ...
        self.set_image(
            self.image_type(self.generate_drr(theta, image_size=image_size, alpha=drr_alpha), generate_mip_levels=True))
        debug.toc()
    ...
    def generate_drr(self, theta, *, alpha: float, image_size: torch.Tensor, mip_level: int = 0) -> torch.Tensor:
        """
        ...
        """
        untransformed_rays = self.ray_type.generate_true_untransformed(image_size, self.source_position,
            device=self.volume.data[mip_level].device)
